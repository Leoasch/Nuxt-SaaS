...
const { errors, resetErrors, handleError } = useFormErrors(
  ['password', 'repeatPassword'] as const,
  'reset_password'
)
...
<template>
  <div class="flex-1 flex flex-col w-full mt-5">
    <h1 class="w-full text-center text-2xl">{{ $t('reset_password') }}</h1>
    <div class="flex flex-col items-center mt-5 gap-4">
      ...
      <UFormField :error="errors.reset_password">
        <div class="w-full flex">
          <UButton
            class="m-auto"
            @click="handleResetPassword">{{ $t('submit_reset_password') }}</UButton>
        </div>
      </UFormField>
    </div>
  </div>
